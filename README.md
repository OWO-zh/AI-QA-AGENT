...
- **云端部署体验**：Streamlit Cloud 一键部署，共享缓存模型与示例知识库索引（开箱即问），首次加载后秒级响应
...
- 演示密码已在登录页公开展示（防滥用软门槛，而非隐私保护），输入即可体验；配合每会话 25 次提问上限防止 API 额度被恶意消耗
- 云端演示内置防护：每会话 25 次提问上限 + 8 秒间隔限制（本地开发不受影响）
- 首页显示功能徽章、快速提问 chips 与 20 条预设问题，内置示例知识库开箱即问
- 首次访问自动加载模型并构建示例知识库索引（约 10~30 秒，spinner 有提示），后续秒级响应
...
| 长时间转圈 | 首次下载/加载模型并构建示例库索引 | 等待 spinner 完成（云端下载约 3-5 分钟），后续秒开 |
...
├── requirements.txt      # 精简核心依赖（16 个包，已锁定版本）
├── assets/
│   └── demogif.gif       # 演示动画
└── docs/
    └── TECHNICAL.md      # 详细技术文档
